    ...
    <>
      ...
      <div className="p-8 bg-gray-100 min-h-screen">
        <Grid placeItems="center">
          <Text as="h1" fontSize="2xl" fontWeight="bold" mb={2} color="red.600">
            Donation Records Dashboard
          </Text>
          <Text mb={7} fontSize="md" color="gray.600" textAlign="center">
            Review and manage all donation records. Utilize the search
            functionality to quickly locate records by donor CIN or name. This
            dashboard allows for the modification or deletion of donation
            entries to keep data up-to-date.
          </Text>
        </Grid>
                ...
                {filteredDonations.map((donation) => (
                  <tr key={donation.id} className="bg-white border-b">
                    ...
                    <td className="px-6 py-4">
                      {donation.donor?.Cin ?? "N/A"}
                    </td>
                    <td className="px-6 py-4">
                      {donation.QuantityDonated ?? "N/A"}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 flex space-x-3">
                      <a
                        ...
                        href={`/editDonation/${donation.id}`}
                      >
                        <Pencil size={20} />
                      </a>
                      <button
                        className="text-red-500 hover:text-red-700"
                        onClick={() => deleteItem(donation.id)}
                      >
                        <Trash2 size={20} />
                      </button>
                    </td>
